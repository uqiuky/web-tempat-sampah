...
$(function () {
  $(".progressIcon").each(function () {
    let divProgress = $("<div></div>")
      .css({ position: "relative", display: "table" })
    ...
    if (animation > 0) {
      ...
      $(this).css(
        "clip-path",
        `inset(0% ${100 - parseInt($(this).data("percent"))}% 0% 0%)`
      );
    }
  });
  ...
  function anim(elem) {
    $(elem)
      .css("clip-path", `inset(100% 0% 0% 0%)`)
      .animate(
        { someValue: parseInt($(elem).data("percent")) },
        {
          duration: parseInt($(elem).data("animation")),
          step: function (someValue) {
            $(elem).css("clip-path", `inset(${100 - someValue}% 0% 0% 0%)`);
          },
        }
      )
      .delay(800);
